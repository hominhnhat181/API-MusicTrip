...
@section('content')
    <div id="pages" class="main_contain">
        <div class="main_contain-hover">
            ...
            <div class="table_view">
                <div class="table_hover">
                    <table class="table table-hover my-0">
                        ...
                        <tbody>
                            @if (!empty($artists) && count([$artists]))
                                @foreach ($artists as $artist)
                                    <tr class="cms-index">
                                        <td class="first-column">{{ $artist->id }}</td>
                                        <td class="d-none d-xl-table-cell">{{ $artist->name }}</td>
                                        <td class="d-none d-xl-table-cell center">
                                            <img style="width: 50px;height:50px;border-radius: 50%;" src="{{ asset($artist->image) }}" alt="">
                                        </td>
                                        <td class="center">
                                            <a href="{{route('admin.artist.status',  $artist->id)}}" data-toggle="modal"
                                                data-target="#ModalCenterS{{ $artist->id }}">
                                                @if ($artist->status != 1)
                                        ...
                                        <td class="d-none d-md-table-cell center">
                                            <div class="ct ">
                                                <a class="btn btn-soft-primary btn-icon btn-circle go-edit btn-sm"
                                                    href="{{ route('admin.artist.edit', $artist->id) }}"
                                                    title="{{ translate('Edit') }}">
                                                    <i class="las la-edit"></i>
                                                </a>
                                                <a href="#"
                                                    class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete"
                                                    data-href="{{ route('admin.artist.destroy', $artist->id) }}"
            ...
            {{ $artists->appends(request()->input())->links('vendor.pagination.default') }}
        </div>
    </div>

    @foreach ($artists as $artist)
<!-- Modal status-->
<div class="modal fade" id="ModalCenterS{{ $artist->id }}" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title" id="exampleModalLongTitle">Change Status Artist</h3>
            ...
            <div class="modal-footer">
                ...
                <a href="{{ Route('admin.artist.status', $artist->id) }}"><button type="button"
        ...
        $(document).ready(function() {
            $('.admin_search-input').keyup(function() { 
                ...
                if (query != '') {
                    ...
                    $.ajax({
                        url: "{{ route('admin.artist.fillSearch') }}", 
                        method: "POST", // 
                        data: {
                            query: query,
                            "_token": "{{ csrf_token() }}"
