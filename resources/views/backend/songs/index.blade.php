                ...
                <div class="row ">
                    ...
                    <div class="admin_search col-xl-3 col-md-5" id="alphay">
                        <select class="admin_search-chose aiz-selectpicker form-control" id="select_user" name="status">
                            ...
                            <option value="1" {{ request('status') == 2 ? 'selected' : '' }}>Active</option>
                            <option value="2" {{ request('status') == 1 ? 'selected' : '' }}>Deactive</option>
                        </select>
                    </div>
                    <?php
                    $request = request()->all();
        ...
        $(document).ready(function() {
            $('.admin_search-input').keyup(function() { //bắt sự kiện keyup khi người dùng gõ từ khóa tim kiếm
                ...
                if (query != '') {
                    ...
                    $.ajax({
                        url: "{{ route('admin.song.fillSearch') }}", // đường dẫn khi gửi dữ liệu đi 'search' là tên route mình đặt bạn mở route lên xem là hiểu nó là cái j.
                        ...
                        success: function(data) { //dữ liệu nhận về
                            $('#objectList').fadeIn();
                            $('#objectList').html(
                            data); //nhận dữ liệu dạng html và gán vào cặp thẻ có id là objectList
                        },
                        error: function(xhr) {
