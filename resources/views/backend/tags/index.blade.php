...
@section('content')
            ...
            <form class="" id=" sort_user" action="" method="GET">
                <div class="row ">
                    ...
                    <div class="admin_search col-xl-5 col-md-12 row" id="omega">
                        ...
                        <a class="admin_search-btn col-md" href="{{ route('admin.tag.index') }}">Reset</a>
                        <a class="admin_search-btn col-md" href="">Excel</a>
                        <a class="admin_search-btn col-md" href="{{ Route('admin.tag.create') }}">Create</a>
                    </div>
                </div>
            </form>
                    ...
                    <table class="table table-hover my-0">
                        ...
                        <tbody>
                            @if (!empty($tags) && count([$tags]))
                                @foreach ($tags as $tag)
                                    <tr class="cms-index">
                                        <td class="first-column">{{ $tag->id }}</td>
                                        <td class="d-none d-xl-table-cell">{{ $tag->name }}</td>
                                        <td class="center">
                                            <a href="{{ route('admin.tag.status', $tag->id) }}" data-toggle="modal"
                                                data-target="#ModalCenterS{{ $tag->id }}">
                                                @if ($tag->status != 1)
                                        ...
                                        <td class="d-none d-xl-table-cell center">{{ $tag->created_at->format('d-m-Y') }}
                                        </td>
                                        <td class="d-none d-md-table-cell center">
                                            <div class="ct ">
                                                ...
                                                <a href="#"
                                                    ...
                                                    data-href="{{ route('admin.tag.destroy', $tag->id) }}"
    ...
    @foreach ($tags as $tag)
        <!-- Modal status-->
        <div class="modal fade" id="ModalCenterS{{ $tag->id }}" tabindex="-1" role="dialog"
            aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h3 class="modal-title" id="exampleModalLongTitle">Change Status Tag</h3>
                    ...
                    <div class="modal-footer">
                        ...
                        <a href="{{ Route('admin.tag.status', $tag->id) }}"><button type="button"
