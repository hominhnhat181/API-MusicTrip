...
@section('content')
    <div id="pages" class="main_contain">
        <div class="main_contain-hover">
            ...
            <div class="table_view">
                <div class="table_hover">
                    <table class="table table-hover my-0">
                        ...
                        <tbody>
                            @if (!empty($albums) && count([$albums]))
                                @foreach ($albums as $album)
                                    <tr class="cms-index">
                                        <td class="first-column">{{ $album->id }}</td>
                                        <td class="d-none d-xl-table-cell">{{ $album->name }}</td>
                                        <td class="d-none d-xl-table-cell">
                                            <img style="width: 50px;height:50px;border-radius: 50%;" src="{{ asset($album->image) }}" alt="">
                                        </td>
                                        <td class="first-column">{{ $album->desc }}</td>
                                        <td class="first-column">{{ $album->features->name }}</td>
                                        <td class="center">
                                            <a href="{{route('admin.album.status',  $album->id)}}" data-toggle="modal"
                                                data-target="#ModalCenterS{{ $album->id }}">
                                                @if ($album->status != 1)
                                        ...
                                        <td class="d-none d-md-table-cell center">
                                            <div class="ct ">
                                                ...
                                                <a href="#"
                                                    ...
                                                    data-href="{{ route('admin.album.destroy', $album->id) }}"
            ...
            {{ $albums->appends(request()->input())->links('vendor.pagination.default') }}
        </div>
    </div>

    @foreach ($albums as $album)
<!-- Modal status-->
<div class="modal fade" id="ModalCenterS{{ $album->id }}" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <a href="{{ Route('admin.album.status', $album->id) }}"><button type="button"
        ...
        $(document).ready(function() {
            $('.admin_search-input').keyup(function() { 
                ...
                if (query != '') {
                    ...
                    $.ajax({
                        url: "{{ route('admin.album.fillSearch') }}", 
                        method: "POST", // 
                        data: {
                            query: query,
                            "_token": "{{ csrf_token() }}"
